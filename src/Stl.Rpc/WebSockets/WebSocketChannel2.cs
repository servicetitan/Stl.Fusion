...
public sealed class WebSocketChannel2<T> : Channel<T>
    where T : class
{
    ...
    public WebSocketChannel2(WebSocket webSocket, CancellationToken cancellationToken = default)
        : this(Options.Default, webSocket, cancellationToken) { }
    public WebSocketChannel2(Options settings, WebSocket webSocket, CancellationToken cancellationToken = default)
    {
        Settings = settings;
        WebSocket = webSocket;
    ...
    {
        var readChannelWriter = _readChannel.Writer;
        try {
            await foreach (var item in ReadAll(cancellationToken).ConfigureAwait(false))
                await readChannelWriter.WriteAsync(item, cancellationToken).ConfigureAwait(false);
            readChannelWriter.TryComplete();
        }
        catch (Exception e) {
            readChannelWriter.TryComplete(e);
            throw;
        }
        finally {
            _ = Close();
