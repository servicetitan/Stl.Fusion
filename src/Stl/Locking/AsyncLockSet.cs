...
namespace Stl.Locking
{
        ...
        where TKey : notnull
    {
        ...
        private readonly ConcurrentDictionary<TKey, TaskSource<Unit>> _locks;
            ...
            int concurrencyLevel, int capacity)
        {
            ...
            _locks = new ConcurrentDictionary<TKey, TaskSource<Unit>>(concurrencyLevel, capacity);
        ...
        {
            var myLock = TaskSource.New<Unit>(_taskCreationOptions);
        ...
        protected IDisposable CreateDisposable(TKey key, 
            TaskSource<Unit> myLock, Dictionary<TKey, int> localLocks)
        {
            if (localLocks != null) {
