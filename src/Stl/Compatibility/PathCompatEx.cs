...
namespace System.IO
{
    internal static class PathCompatEx
    {
        ...
        private static bool PartiallyQualified(string path)
        {
            if (path.Length < 2)
                return true;
            return IsDirectorySeparator(path[0]) ? path[1] != '?' && !IsDirectorySeparator(path[1]) : path.Length < 3 || path[1] != ':' || !IsDirectorySeparator(path[2]) || !IsValidDriveChar(path[0]);
        }
        ...
        {
            // implementation is taken from https://stackoverflow.com/questions/3139467/how-to-get-absolute-file-path-from-base-path-and-relative-containing
