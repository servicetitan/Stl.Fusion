...
namespace Stl.ImmutableModel.Reflection
{
    public class NodeTypeDef
    {
        ...
        public IReadOnlyDictionary<Symbol, INodePropertyDef> FrozenProperties { get; }
        ...
        public NodeTypeDef(Type type)
        {
            ...
            FrozenProperties = new ReadOnlyDictionary<Symbol, INodePropertyDef>(
                properties.Where(p => p.Value.IsFrozen).ToDictionary());
        }
        ...
        public virtual void GetFrozenItems(INode node, ref MemoryBuffer<KeyValuePair<ItemKey, IFrozen>> output)
        {
            foreach (var (name, propertyDef) in FrozenProperties) {
                var getter = (Func<INode, object>?) propertyDef.UntypedGetter;
                if (getter == null)
                    continue;
                var value = getter.Invoke(node);
