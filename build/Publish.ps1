...
function Publish($name) {
    ...
    dotnet nuget push "artifacts/nupkg/$name.$nupkgVersion.snupkg" -k $env:NUGET_ORG_API_KEY -s https://api.nuget.org/v3/index.json
...
function PublishAll() {
    ...
    foreach ($package in Get-ChildItem "artifacts/nupkg/*.snupkg" ) {
        dotnet nuget push $package -k $env:NUGET_ORG_API_KEY -s https://api.nuget.org/v3/index.json
    }
}
