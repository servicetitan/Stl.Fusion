    ...
    public class ComputedInterceptorTest : FusionTestBase, IAsyncLifetime
    {
        ...
        public async Task AutoRecomputeTest()
        {
            ...
            var c = await Computed.CaptureAsync(
                _ => time.GetTimeOffsetAsync(TimeSpan.FromSeconds(1)));
        ...
        public async Task InvalidationAndCachingTest2()
        {
            ...
            var c1 = await Computed.CaptureAsync(_ => time.GetTimeOffsetAsync(TimeSpan.FromSeconds(1)));
            c1.Should().NotBeNull();
            var c2 = await Computed.CaptureAsync(_ => time.GetTimeOffsetAsync(TimeSpan.FromSeconds(2)));
            c2.Should().NotBeNull();
            c1.Should().NotBeSameAs(c2);

            var c1a = await Computed.CaptureAsync(_ => time.GetTimeOffsetAsync(TimeSpan.FromSeconds(1)));
            var c2a = await Computed.CaptureAsync(_ => time.GetTimeOffsetAsync(TimeSpan.FromSeconds(2)));
            ...
            c1a = await Computed.CaptureAsync(_ => time.GetTimeOffsetAsync(TimeSpan.FromSeconds(1)));
            c2a = await Computed.CaptureAsync(_ => time.GetTimeOffsetAsync(TimeSpan.FromSeconds(2)));
