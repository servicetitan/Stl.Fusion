...
{
    public class RefHashSetSlimTest
    {
        private string[] Items { get; } =
            Enumerable.Range(0, 52).Select(i => ((char) ('a' + i % 26)).ToString()).ToArray();

        [Fact]
        public void CombinedTest()
        {
            Items.Distinct().Count().Should().Be(26);
            var hs = new HashSet<string>(Items, ReferenceEqualityComparer<string>.Instance);
            hs.Count.Should().Be(52);

            Test(new RefHashSetSlim1<string>());
            Test(new RefHashSetSlim2<string>());
            Test(new RefHashSetSlim3<string>());
            Test(new RefHashSetSlim4<string>());
        }

        private void Test(IRefHashSetSlim<string> c)
        {
            var hs = new HashSet<string>(ReferenceEqualityComparer<string>.Instance);
            var rnd = new Random();
